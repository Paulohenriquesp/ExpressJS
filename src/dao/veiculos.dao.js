...
export const getVeiculos = async (page, size) => {
  ...
  const veiculos = await db("veiculos").select("*").limit("size").off(offset);

  return veiculos;
};

export const getVeiculoById = async (id) => {
  const veiculo = await db("veiculos").select("*").where({ id }).first();

  return veiculo;
};

export const createVeiculo = async (veiculo) => {
  const [id] = await db("veiculos").insert(veiculo);
...
export const deleteVeiculo = async (id) => {
  await db("veiculos").where({ id }).delete();

  return { message: `Veículo com ID ${id} foi excluído com sucesso.` };
};
